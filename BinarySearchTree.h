...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
      ...
      copyNode->datum = node->datum;
      copyNode->left = node->left;
      return copy_nodes_impl(copyNode->left);
      copyNode->right = node->right;
      return copy_nodes_impl(copyNode->right);
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    else if(!less(node->left->datum,node->datum)
      ...
      if (node->left != nullptr && node->right != nullptr) {
          ...
          if (less(val, node->datum)) {
              ...
              if (!less(val, node->left->datum) &&
                  min_greater_than_impl(node->left, val, less) == nullptr)
